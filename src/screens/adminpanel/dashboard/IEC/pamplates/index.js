...
const AdminPdf = ({navigation}) => {
  ...
  const getPamplateId = async id => {
    const url = API.GET_PAMPLATE_BYID + id;
    console.log({url});
    try {
      setIsLoading(true);
      const result = await getApi(url);
  ...
  const deletePamplate = async id => {
    const url = API.DELETE_PAMPLATE + id;
    console.log({url});
    try {
      setIsLoading(true);
  ...
  const update_pamplate = async () => {
    const url = API.UPDATE_PAMPLATE + pamplateId;
    console.log({url});
    const params = {
  ...
  const ListHeaderComponent = () => {
    return (
      <View style={st.mt_B}>
        <Text style={[st.tx16, {color: colors.black}]}>{'Pamphlet'}</Text>
      </View>
    );
  };
