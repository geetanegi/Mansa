  ...
  const saveScoreHandle = async ageId => {
    ...
    try {
      ...
      if (result?.status == 201) {
        const data = result.data;
        setIsLoading(false);
        navigation.navigate('SA_suggestion', {score: score, ageId: ageId, childDetails:childDetails});
      } else {
        setIsLoading(false);
      }
    } catch (e) {
              ...
              <View>
                {lang == 'hi' ? (
                  <Text style={[st.question_heading]}>
                    निम्नलिखित प्रश्न कुछ तकलीफों और समस्याओं से संबंधित लक्षण
                    हैंं। हो सकता हैं इनमें से कुछ लक्षणों ने
                    <Text style={[st.txbold, {color: colors.danger}]}>
                      {' '}
                      पिछले 30 दिनों{' '}
                    </Text>
                    में आपको परेशान किया हो। यदि आपको लगता हैं कि कोई प्रश्न आप
                    पर लागू होता हैं जिसमें
                    <Text style={[st.txbold, {color: colors.danger}]}>
                      {' '}
                      पिछले 30 दिनों
                    </Text>{' '}
                    आपको महसूस हुई समस्या को दर्शाया हैं तो "हाँ" पर क्लिक करे ।
                    दूसरी ओर, यदि प्रश्न आप पर लागू नहीं होता हैं और
                    <Text style={[st.txbold, {color: colors.danger}]}>
                      {' '}
                      पिछले 30 दिनों
                    </Text>{' '}
                    में आपको कोई समस्या नहीं हुई हैं, तो “नहीं” पर क्लिक करे।
                  </Text>
                ) : (
                  <Text style={[st.question_heading]}>
                    ...
                    answer YES. On the other hand, if the question does not
                    apply to you and you did not have the problem in the{' '}
                    <Text style={[st.txbold, {color: colors.danger}]}>
                      last 30 days,
                    </Text>{' '}
                    answer NO.
